...
function renderCart() { // 渲染购物车、金额、region 下拉、红色提醒
  ...
  const shippingWarning = textSetting(
    "Delivery in about 12 business days",
    "Please select the region that matches your shipping address." // 默认红色提醒；建议去 Settings 表用 shipping.warning 改
  );
...
createOrder: async () => { // 创建 PayPal 订单
  ...
  return data.paypalOrderId;
    ...
    onError: (error) => { // PayPal 错误
      console.error(error);
      alert("Please select shipping region before checkout.");
    }
